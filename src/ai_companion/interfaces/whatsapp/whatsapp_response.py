...
async def download_media(media_id: str) -> bytes:
    ...
    metadata = await safe_http_get(media_metadata_url, headers=headers)
    if not metadata:
    ...
    if download_url:
        ...
        media_bytes = await safe_http_get(sanitized_download_url, headers=headers)
...
async def process_audio_message(message: Dict) -> str:
    ...
    if not URLValidator.is_valid(download_url):
        ...
        return ""
    audio_bytes = await safe_http_get(download_url, headers=headers)
    if not audio_bytes:
